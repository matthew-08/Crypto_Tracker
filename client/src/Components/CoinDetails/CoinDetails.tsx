...
export function CoinDetails({
  ...
  const [percentageRows, setPercentageRows] = useState(
    [] as MarketPercentage[][]
  );
        ...
        const array: MarketPercentage[] = [
          {
            ...
            percentage:
              coinDetails.marketData.percent1h[coin as keyof PercentageCoins],
            type: 'percentage',
          },
          {
            label: `24h ${coin.toUpperCase()}`,
            percentage:
              coinDetails.marketData.percent24h[coin as keyof PercentageCoins],
            type: 'percentage',
          },
          {
            label: `7d ${coin.toUpperCase()}`,
            percentage:
              coinDetails.marketData.percent7d[coin as keyof PercentageCoins],
            type: 'percentage',
          },
          {
            label: `30d ${coin.toUpperCase()}`,
            percentage:
              coinDetails.marketData.percent30d[coin as keyof PercentageCoins],
            type: 'percentage',
          },
        ];
  ...
  return (
    <section className={styles['main-container']}>
      <header className={styles.header}>
        <div className={styles['header-top']}>
          <div className={styles.logo}>
            ...
            <div className={styles['coin-name-container']}>
              <p>{coinDetails.name}</p>
              <small>{coinDetails.symbol.toUpperCase()}</small>
            </div>
          </div>
          <div className={styles['main-price-container']}>
      ...
      <div className={styles['container-bottom']}>
        <div className={styles['market-percentages-container']}>
          {percentageRows &&
            percentageRows.map((row) => <Row key={uuid()} arrayOfData={row} />)}
        </div>
        <div className={styles['bottom-info-right']}>
          <Row
            key={uuid()}
            arrayOfData={[
              {
      ...
      <div className={styles.footer}>
        <SocialLinks links={coinDetails.marketData.links} />
